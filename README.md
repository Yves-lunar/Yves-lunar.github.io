...
项目按最近保存时间排列，新建项目也会参与排序。点击默认项目标题会自动选中文字，可直接输入新名称。正文下方的工具栏支持加粗、下划线、删除线、有序/无序列表、待办方框和六种文字颜色；选中文字后点击工具，或点击「预览」查看效果、勾选方框，最后点击「保存修改」。下划线和颜色使用受限 HTML 标记，其余使用 Markdown，预览不会执行正文中的 HTML 或脚本。

项目编辑时间以兼容标记保存在正文开头，由存储层自动隐藏；原有纯文本项目直接兼容，创建时间保持不变。此次更新请一并上传 `supabase-store.js`、`notebook-model.js`、`notebook-ui.js`、`notebook.css`，无需修改数据库。更早的项目没有历史编辑时间，第一次编辑前按创建时间排列。
...
运行 `node --test tests/supabase-store.test.cjs tests/notebook-model.test.cjs`。15 项测试覆盖共享读写、分页、流水账兼容保存、归档分组、最近 4 条筛选、项目排序及正文格式。
